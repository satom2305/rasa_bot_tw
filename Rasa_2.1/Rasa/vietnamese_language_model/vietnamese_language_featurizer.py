...
class VietnameseLanguageModelFeaturizer(DenseFeaturizer, GraphComponent):
    ...
    def __init__(
        ...
        super(VietnameseLanguageModelFeaturizer, self).__init__(
    ...
    ) -> VietnameseLanguageModelFeaturizer:
    ...
    def _load_model_instance(self) -> None:
        ...
        #PhobertTokenizer.from_pretrained(self.model_weights)
        logger.debug(f"Loading Tokenizer and Model for VietnameseLanguageModelFeaturizer using {self.model_name}")
        ...
        logger.debug(f"Loading TFRobertaModel for VietnameseLanguageModelFeaturizer at {self.model_weights}")
        self.model = TFRobertaModel.from_pretrained(self.model_weights, cache_dir=self.cache_dir)
        
        logger.debug(f"Success Loaded TFRobertaModel for VietnameseLanguageModelFeaturizer at {self.model_weights}")
...
#        )
    ...
    def _lm_specific_token_cleanup(
        ...
        from rasa.nlu.utils.hugging_face.transformers_pre_post_processors import gpt2_tokens_cleaner

        return gpt2_tokens_cleaner(split_token_ids, token_strings)
    ...
    def _process_message(self, message: Message) -> Message:
        ...
        #        doc=self._get_docs_for_batch(
        #                [message], attribute=attribute, inference_mode=True
        #            )[0]
        #        print("sequence_features=",doc[SEQUENCE_FEATURES])
        #        print("sentence_features=",doc[SENTENCE_FEATURES])
        #print("message.features=",message.features)
        return message
